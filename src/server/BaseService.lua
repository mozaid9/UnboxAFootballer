...
-- ── Fan Park ─────────────────────────────────────────────────
local function createFanPark(parent, centerZ)
	local park = make("Model", { Name = "FanPark" }, parent)
	...
	make("Part", {
		...
		Anchored = true, CanCollide = true,
		Material = Enum.Material.Slate,
		Color = Color3.fromRGB(32, 42, 26),
		Size = Vector3.new(36, 0.5, 36),
		CFrame = CFrame.new(0, 0.55, centerZ),
	}, park)
	for _, edge in ipairs({
		{ pos = Vector3.new(0, 0.87, centerZ + 18),  sz = Vector3.new(36.4, 0.22, 0.22) },
		{ pos = Vector3.new(0, 0.87, centerZ - 18),  sz = Vector3.new(36.4, 0.22, 0.22) },
		{ pos = Vector3.new(-18, 0.87, centerZ),     sz = Vector3.new(0.22, 0.22, 36.4) },
		{ pos = Vector3.new(18, 0.87, centerZ),      sz = Vector3.new(0.22, 0.22, 36.4) },
	}) do
		make("Part", {
			Name = "FanParkDeckEdge",
			Anchored = true, CanCollide = false,
			Material = Enum.Material.Neon,
			Color = Color3.fromRGB(80, 220, 80),
			Transparency = 0.5,
			Size = edge.sz,
			CFrame = CFrame.new(edge.pos),
		}, park)
	end

	-- Central glowing orb on pedestal
	make("Part", {
		Name = "FanParkPedestal",
		Anchored = true, CanCollide = false,
		Material = Enum.Material.Slate,
		Color = Color3.fromRGB(38, 50, 32),
		Size = Vector3.new(3.5, 1.4, 3.5),
		CFrame = CFrame.new(0, 1.5, centerZ),
	}, park)
	local parkOrb = make("Part", {
		Name = "FanParkOrb",
		Anchored = true, CanCollide = false,
		Material = Enum.Material.Neon,
		Color = Color3.fromRGB(255, 215, 0),
		...
		CFrame = CFrame.new(0, 3.85, centerZ),
		Transparency = 0.22,
	}, park)
	make("PointLight", {
	...
	-- 4 flag poles with neon flags
	for _, c in ipairs({ { -12, -12 }, { 12, -12 }, { -12, 12 }, { 12, 12 } }) do
		local px, pz = c[1], c[2]
		local pole = make("Part", {
			Name = "FanParkPole",
			Anchored = true, CanCollide = false,
			...
			CFrame = CFrame.new(px, 5.8, centerZ + pz),
		}, park)
		make("Part", {
			Name = "FanParkFlag",
			Anchored = true, CanCollide = false,
			...
			Color = Color3.fromRGB(80, 220, 80),
			Transparency = 0.3,
			Size = Vector3.new(0.1, 2.4, 4),
			CFrame = CFrame.new(px + 2, 10, centerZ + pz),
		}, park)
		make("PointLight", {
			Color = Color3.fromRGB(160, 255, 130),
			Brightness = 0.65,
			Range = 20,
			Shadows = false,
		}, pole)
	...
	-- 4 benches facing the orb
	for _, b in ipairs({
		{ x = -10, z = 0,   ry = 0 },
		{ x = 10,  z = 0,   ry = 0 },
		{ x = 0,   z = -10, ry = math.pi / 2 },
		{ x = 0,   z = 10,  ry = math.pi / 2 },
	}) do
		make("Part", {
			Name = "FanParkBench",
			Anchored = true, CanCollide = true,
			Material = Enum.Material.Wood,
			Color = Color3.fromRGB(158, 102, 52),
			Size = Vector3.new(5.5, 0.32, 1.4),
			CFrame = CFrame.new(b.x, 1.22, centerZ + b.z) * CFrame.Angles(0, b.ry, 0),
		}, park)
	end

	-- "FAN PARK" sign board
	local fpSign = make("Part", {
		Name = "FanParkSign",
		Anchored = true, CanCollide = false,
		Material = Enum.Material.Slate,
		Color = Color3.fromRGB(18, 28, 14),
		Size = Vector3.new(16, 3.5, 0.5),
		CFrame = CFrame.new(0, 3.4, centerZ - 21),
	}, park)
	local fpGui = make("SurfaceGui", {
		Face = Enum.NormalId.Front,
		SizingMode = Enum.SurfaceGuiSizingMode.PixelsPerStud,
		PixelsPerStud = 50,
		AlwaysOnTop = false,
	...
	make("TextLabel", {
		...
		BackgroundTransparency = 1,
		Text = "RELAX  •  CELEBRATE  •  VIBE",
		Font = Enum.Font.GothamMedium,
		TextColor3 = Color3.fromRGB(200, 200, 200),
		TextScaled = true,
	}, fpGui)
	make("Part", {
		Name = "FanParkSignGlow",
		Anchored = true, CanCollide = false,
		Material = Enum.Material.Neon,
		Color = Color3.fromRGB(55, 180, 55),
		Transparency = 0.7,
		Size = Vector3.new(16.8, 4, 0.2),
		CFrame = CFrame.new(0, 3.4, centerZ - 21.2),
	}, park)
...
local function createHypePark(parent, centerZ)
	...
	make("Part", {
		...
		Size = Vector3.new(40, 0.5, 48),
		CFrame = CFrame.new(0, 0.55, centerZ),
	}, park)
	-- Checker tile accents on crowd floor
	local jamTileColors = { Color3.fromRGB(50, 20, 70), Color3.fromRGB(20, 40, 60) }
	local ti = 0
	for zi = 0, 2 do
		for xi = 0, 2 do
			ti += 1
			make("Part", {
				...
				Transparency = 0.82,
				Size = Vector3.new(11.8, 0.1, 11.8),
				CFrame = CFrame.new(-11 + xi * 11, 0.82, centerZ - 11 + zi * 11),
			}, park)
		end
	end
	...
	make("Part", {
		...
		Size = Vector3.new(34, 2.5, 20),
		CFrame = CFrame.new(0, 1.55, stageZ),
	}, park)
	...
	local stageLip = make("Part", {
		...
		Transparency = 0.25,
		Size = Vector3.new(34.4, 0.25, 0.25),
		CFrame = CFrame.new(0, 2.92, stageZ - 10.12),
	}, park)
	make("PointLight", {
	...
	local jamBackdrop = make("Part", {
		...
		Size = Vector3.new(34, 14, 0.9),
		CFrame = CFrame.new(0, 10.3, stageZ + 10.45),
	}, park)
	...
	for i, sc in ipairs({ Color3.fromRGB(220, 50, 255), Color3.fromRGB(50, 200, 255), Color3.fromRGB(255, 80, 130) }) do
		local stripPart = make("Part", {
			...
			Color = sc,
			Transparency = 0.28,
			Size = Vector3.new(34, 0.35, 0.22),
			CFrame = CFrame.new(0, 4.8 + i * 2.6, stageZ + 10.5),
		}, park)
		make("PointLight", {
			Color = sc,
			Brightness = 0.7,
	...
	for _, sx in ipairs({ -19, 19 }) do
		local tower = make("Part", {
			...
			CFrame = CFrame.new(sx, 9.3, stageZ),
		}, park)
		make("PointLight", {
		...
		for ri, rc in ipairs({ Color3.fromRGB(220, 50, 255), Color3.fromRGB(50, 200, 255) }) do
			make("Part", {
				...
				Color = rc,
				Transparency = 0.4,
				Size = Vector3.new(3.7, 0.25, 3.7),
				CFrame = CFrame.new(sx, 4.5 + ri * 3.5, stageZ),
			}, park)
		end
		...
		local spotAnchor = make("Part", {
			...
			Transparency = 1,
			Size = Vector3.new(1, 1, 1),
			CFrame = CFrame.new(sx, 15.8, stageZ),
		}, park)
		make("SpotLight", {
			Brightness = 6,
			Range = 50,
			Angle = 35,
			Color = Color3.fromRGB(240, 180, 255),
			Shadows = false,
			Face = Enum.NormalId.Bottom,
		}, spotAnchor)
	end
